...
"""
Performance-optimized version of ADS_bin with enhanced GPU management, model caching,
and comprehensive performance tracking. Includes memory optimization, progress tracking,
and improved error handling for clinical deployment.
"""
...
class GPUManager:
    """Manage GPU configuration and detection"""
        ...
        try:
            ...
            if gpus:
                for i, gpu in enumerate(gpus):
                    ...
                    print(f"GPU {i} Memory - Current: {memory_info['current']//1024//1024}MB, Peak: {memory_info['peak']//1024//1024}MB")
        except:
            pass
        
        # System memory
        memory = psutil.virtual_memory()
        print(f"System Memory - Used: {memory.used//1024//1024}MB ({memory.percent:.1f}%), Available: {memory.available//1024//1024}MB")
...
class ModelManager:
    ...
    def load_model(self, model_path, model_name):
        ...
        if model_name not in self._models:
            ...
            self._models[model_name] = load_model(model_path, compile=False)
...
class PerformanceTracker:
    ...
    def get_summary(self):
        ...
        print(f"{'='*50}")
        print(f"{'Total Processing Time':<30}: {total_time:6.2f}s")
        return self.metrics
...
def get_MaskNet_MNI(model, Dwi_MNI_img, B0_MNI_img, verbose=False):
    ...
    y_pred = model.predict(x, verbose=0 if not verbose else 1)
...
def get_stroke_seg_MNI(model, dwi_img, adc_img, Prob_IS=None, N_channel=3, DS=2, show_progress=False):
    stroke_pred_resampled =  np.squeeze(np.zeros_like(dwi_img))
    
    # Create iterator for progress tracking
    iterator_list = [(x,y,z) for x in range(DS) for y in range(DS) for z in range(2*DS)]
    
    if show_progress:
        iterator = tqdm(iterator_list, desc="Processing segments", leave=False)
    else:
        iterator = iterator_list
        
    for x_idx, y_idx, slice_idx in iterator:
        if N_channel==3:
...
def ADS_pipeline(SubjDir, 
                 ...
                 show_progress=False,  # New parameter for progress tracking
                 performance_tracker=None  # New parameter for performance tracking
                ):
    ...
    performance_tracker.start_timer("Total Pipeline")
    performance_tracker.start_timer("Data Loading")
    ...
    if not os.path.exists(ADCPath):
        print(f'Calculating ADC with b-value={bvalue}...')
    ...
    performance_tracker.end_timer("Data Loading")
    
    # mapping to MNI with skull for MaskNet
    performance_tracker.start_timer("MNI Registration 1")
    ...
    reg_time = performance_tracker.end_timer("MNI Registration 1")
    print(f'MNI registration completed in {reg_time:.2f}s')
    
    Dwi_MNI_img = affine_map.transform(Dwi_img)
    
    # Loading MaskNet
    performance_tracker.start_timer("Model Loading")
    print('Loading MaskNet...')
    MaskNet = model_manager.load_model(MaskNet_name, "BrainMaskNet")
    performance_tracker.end_timer("Model Loading")
    
    # get brain mask in raw space
    performance_tracker.start_timer("Brain Mask Inference")
    print('Inferencing brain mask...')
    mask_MNI_img = get_MaskNet_MNI(MaskNet, Dwi_MNI_img, B0_MNI_img, verbose=show_progress)
    ...
    mask_time = performance_tracker.end_timer("Brain Mask Inference")
    print(f'Brain mask completed in {mask_time:.2f}s')
    ...
    performance_tracker.start_timer("MNI Registration 2")
    ...
    reg_time2 = performance_tracker.end_timer("MNI Registration 2")
    print(f'Second MNI registration completed in {reg_time2:.2f}s')
    
    # get normalized dwi
    performance_tracker.start_timer("DWI Normalization")
    ...
    performance_tracker.end_timer("DWI Normalization")
    
    # load detection model
    performance_tracker.start_timer("Lesion Model Loading")
    print('Loading detection model...')
    Lesion_model = model_manager.load_model(Lesion_model_name, os.path.basename(Lesion_model_name))
    performance_tracker.end_timer("Lesion Model Loading")
    
    # get lesion prediction
    performance_tracker.start_timer("Lesion Inference")
    ...
    lesion_time = performance_tracker.end_timer("Lesion Inference")
    print(f'Lesion inference completed in {lesion_time:.2f}s')
 
    # Save outputs
    performance_tracker.start_timer("Output Generation")
    ...
    output_time = performance_tracker.end_timer("Output Generation")
    total_time = performance_tracker.end_timer("Total Pipeline")
    
    print(f'Output generation completed in {output_time:.2f}s')
...
def ADS(SubjDir,
         ...
         show_progress=False,  # New parameter for progress tracking
         clear_models_after=False  # New parameter for memory management
        ):
    ...
    # Show initial memory info if progress tracking is enabled
    if show_progress:
        print("\nSystem Information:")
        GPUManager.get_memory_info()
        print()
    ...
    try:
        performance_tracker = ADS_pipeline(SubjDir,
                 SubjID,
        ...
        # Show performance summary if progress tracking is enabled
        if show_progress:
            performance_tracker.get_summary()
            
        # Clear models from memory if requested
        if clear_models_after:
            model_manager = ModelManager()
        ...
        return ResultsDir, performance_tracker
